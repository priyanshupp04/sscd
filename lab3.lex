// 3.Design, develop and implement C program to construct Predictive / LL(1) ParsingTable for the grammar rules: A ->aBa , B ->bB | '\0'. Use this table to parsethesentence: abba$. /* A ->aBa , B ->bB | @*/
...
for(j=0;j<k;j++)
if(first[i][j]!='@')
strcpy(table[numr(prod[i][0])][numr(first[i][j])],prod[i]); else
...
if(input[i-1]!='$')
{
printf("\n\nInput String Entered Without End marker $"); exit(0);
}
push('$');
push('A');
i=0;
