...
printf("\n $%s\t\t%s$\t\t%s",stack,ip_sym,act); /* second print with action
shift*/
strcpy(act,"shift ");if (ip_sym[ip_ptr]=='(' || ip_sym[ip_ptr]=='*' ||
ip_sym[ip_ptr]==')')
{
temp[0]=ip_sym[ip_ptr];
temp[1]='\0';
...
if (stack[st_ptr]==')' && stack[st_ptr-1]=='E' &&
{
stack[st_ptr-2]='F';
stack[st_ptr-1]='\0';
flag=1;
...
if (stack[st_ptr]=='F' && stack[st_ptr-1]=='*' &&
{
// stack[st_ptr-2]='T';
stack[st_ptr-1]='\0';
st_ptr= st_ptr-2;
...
if( stack[st_ptr]=='T' && stack[st_ptr-1]=='+' &&
ip_sym[ip_ptr]!='*' )
{
//stack[st_ptr-2]='E';
stack[st_ptr-1]='\0';
...
stack[st_ptr-2]=='(')
stack[st_ptr-2]=='T')
stack[st_ptr-2]=='E' &&printf("\n $%s\t\t%s$\t\tE->E+T",stack, ip_sym);
}
else
if ((stack[st_ptr]=='T' && ip_sym[ip_ptr]== '+') || (stack[0]=='T' &&
ip_sym[ip_ptr]== '\0') ||
